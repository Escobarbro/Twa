...
tkn = "7066676369:AAGt9df7Ak5YOTII4Zm0pC0R4rK7L7WkSO8"
my_id = int("6934572688")
bot = TeleBot(tkn)
...
n = """• *اهلا بك في بوت التواصل ❤️*

- ارسل رسالتك بهويه مجهوله وسوف يرد عليك البوت باقرب وقت 📢"""
cr.execute(
...
def start_executor(message: types.Message):
  ...
  if com == "تفعيل":
    ...
    if info.status == "creator" or info.status == "administrator" or ADMIN == my_id:
      ...
      if not Is_activation(message.chat.id):
        ...
        if all_pre(bot, message.chat.id):
          ...
          bot.reply_to(message=message,
                       text=Decor(text="عذرا, يجب اعطائي كافه الصلاحيات !",
                                  type="b"),
                       parse_mode="HTML")
      else:
      ...
      bot.reply_to(message=message,
                   text=Decor(
                     text="عذرا, هذا الاجراء مسموح به للرأساء والأمراء فقط! ",
                     type="b"),
                   parse_mode="HTML")
